<?php
// 搜索用当前组件
Yii::$app->loadPluginComponentView('order/com-search');
// 这里也是公共view组件
                ...
                <el-card
                    ...
                    <div slot="header" class="com-order-head" flex="cross:center">
                        ...
                        <div class="com-order-offline" flex="dir:left wrap:wrap">
                            <template v-if="!!item.status && item.cancel_status == 2 && item.status == 1 && isShowCancel">
                        ...
                        <el-button
                                v-if="isShowCancel && (!!item.status) && item.is_send == 0 && item.cancel_status == 0 && item.is_cancel_show == 1 && item.status == 1"
                                style="right: 60px"
                                class="com-order-del"
                        ...
                        <div class="goods-item" :style="{width: orderTitle[0].width}">
                            <div class="goods" v-for="goods in item.detail">
                                ...
                                <img :src="goods.goods_info && goods.goods_info.goods_attr && goods.goods_info.goods_attr.pic_url ? goods.goods_info.goods_attr.pic_url : goods.goods_info.goods_attr.cover_pic"
                                     class="goods-image">
<!--                                <span v-if="goods.refund_status && goods.refund_status != 3 && (goods.refund && goods.refund.type == 2 && goods.refund.is_confirm == 0) || (goods.refund && (goods.refund.type == 1 || goods.refund.type == 0) && goods.refund.is_refund == 0)"-->
<!--                                      class="com-order-refund-status">售后中</span>-->
<!--                                <span v-else-if="goods.refund_status == '2' && (goods.refund.type == 1 || goods.refund.type == 0) && item.status != 0 && goods.refund.is_refund == 1"-->
<!--                                      class="com-order-refund-status">已退款</span>-->
<!--                                <span v-else-if="goods.refund_status == '2' && goods.refund.type == 2 && item.status != 0"-->
<!--                                      class="com-order-refund-status">已换货</span>-->
<!--                                <span v-else-if="goods.refund_status == '3' && item.status != 0"-->
<!--                                      class="com-order-refund-status">已拒绝</span>-->
                                <span v-if="goods.refund_status == '11' || goods.refund_status == '10' || goods.refund_status == '12'"
                                      class="com-order-refund-status">售后中</span>

                                <span v-else-if="goods.refund_status == '20'"
                                      class="com-order-refund-status">已退款</span>

                                <span v-else-if="goods.refund_status == '21'" class="com-order-refund-status">已拒绝</span>
                                <div flex="dir:left">
                                    <div class="goods-info" style="position: relative;">
										...
										<div v-if="goods.refund_status && goods.refund_status != 3 && (goods.refund && goods.refund.type == 2 && goods.refund.is_confirm == 0) || (goods.refund && (goods.refund.type == 1 || goods.refund.type == 0) && goods.refund.is_refund == 0)"
											style="margin-bottom: 24px;position: absolute;top: 60px;">
											<el-button type="info" @click="toAfterSale" style="margin-right: 10px; padding: 3px 6px;">售后列表</el-button>
                                        </div>
                            ...
                            <div flex="wrap:wrap cross:center">
                                ...
                                <el-tooltip class="item" effect="dark" content="确认收货" placement="top">
                                    <img class="com-order-icon" src="statics/img/mall/order/confirm.png" alt=""
                                         v-if="item.is_recycle == 0 && item.is_send == 1 && item.is_confirm == 0 && isShowConfirm && item.status==2 && item.is_confirm_show"
                                ...
                                <el-tooltip class="item" effect="dark" content="核销订单" placement="top">
                                    <img class="com-order-icon" @click="openDialog(item, clerkVisible = true)"
                                         v-if="item.send_type == 1 && (item.is_pay == 1 || item.pay_type == 2) && item.clerk == null && item.is_send == 0 && item.is_clerk_show && item.is_recycle == 0 && isShowClerk && item.is_recycle == 0 && (!item.status) && item.cancel_status != 1"
                                         src="statics/img/mall/order/clerk.png" alt="">
                                </el-tooltip>
								
								<!--　status==1　&& active.status==2 发货　-->
                                <template v-if="(item.status && (!!item.active) && (!!item.active.status)) && (item.active.status==2) && (item.status==1)">
                                    ...
                                    <!-- <el-tooltip class="item" effect="dark" content="发货" placement="top">
                                        <img class="com-order-icon" @click="openCity(item,'send')"
                                             v-if="item.send_type == 2 && item.is_send == 0 && item.cancel_status != 1  && item.is_send_show == 1 && isShowSend && item.is_recycle == 0"
                                             src="statics/img/mall/order/send.png" alt="">
                                    </el-tooltip>
                                    
                                    <el-tooltip class="item" effect="dark" content="发货" placement="top">
                                        <img class="com-order-icon" @click="storeOrderSend(item)"
                                             v-if="item.send_type == 1  && item.is_send == 0 && item.cancel_status != 1 && item.is_send_show == 1 && isShowSend && item.is_recycle == 0"
                                             src="statics/img/mall/order/send.png" alt="">
                                    </el-tooltip> -->
                                </template>
								...
								<!-- 售后详情 -->
								<!-- <el-tooltip v-if="goods.refund_status && goods.refund_status != 3 && (goods.refund && goods.refund.type == 2 && goods.refund.is_confirm == 0) || (goods.refund && (goods.refund.type == 1 || goods.refund.type == 0) && goods.refund.is_refund == 0)"
									class="item" effect="dark" content="售后详情" placement="top">
                                ...
                                <template v-if="item.send_type != 2">
                                    <template
                                            v-if="item.detailExpress && item.detailExpress.length == 1 && item.is_send == 1 && item.cancel_status != 1 && item.is_confirm == 0 && item.is_recycle == 0 && isShowSend && (!item.status)">
                                    ...
                                    <!--多个物流信息订单 需到订单详情修改 -->
                                    <template
                                            v-else-if="item.detailExpress && item.detailExpress.length >= 1 && item.cancel_status != 1 && item.is_confirm == 0 && item.is_recycle == 0 && isShowSend && (!item.status)">
                                        <el-tooltip class="item" effect="dark" content="修改快递单号" placement="top">
                                            <img class="com-order-icon"
                                ...
                                <el-tooltip class="item" effect="dark" content="修改配送员" placement="top">
                                    <img class="com-order-icon"
                                         v-if="item.send_type == 2 && item.cancel_status != 1 && item.is_confirm == 0 && item.city_info && item.is_recycle == 0 && isShowSend && (!item.status)"
...
<script>
    Vue.component('com-orders-list', {
        ...
        props: {
            ...
            orderUrl: {
                type: String,
                default: 'plugin/group_buy/mall/order/index',
            },
            // 删除回收站请求URl
            recycleUrl: {
                type: String,
            ...
            orderDetailUrl: {
                type: String,
                default: 'plugin/group_buy/mall/order/detail'
            },
            // 修改备注请求URL
            editRemarkUrl: {
                type: String,
        ...
        methods: {
			// 0.0.1 跳转售后
			toAfterSale(){
				navigateTo({r: 'mall/order/refund',})
				// uni.navigateTo({
				// 	url: '/view/mall/order/refund'
				// })
			},
            ...
            toSearch(searchParams) {
				...
				if(searchParams.active_status&&searchParams.active_status==1){
