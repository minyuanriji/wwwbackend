...
class GrafikaOption extends ApiGrafika
{
    ...
    public function getPoster(array $option)
    {
        ...
        $array = parse_url($this->qrcode_url);
        $path = $array["path"];
        $filePath = substr(ROOT_PATH,0,-3).substr(dirname($path),1).'/';
        $file = $filePath.basename($path);
        $this->qrcode_url = CommonLogic::uploadImgToCloudStorage($file,$path,$this->qrcode_url);
        return $this;
    }
}