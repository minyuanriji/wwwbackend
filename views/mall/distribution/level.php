        ...
        <div slot="header">
            <div>
                ...
                <div style="float: right; margin: -5px 0">
                    <el-button type="primary" @click="$navigate({r: 'mall/distribution/level-edit'})" size="small">添加分销商等级</el-button>
                </div>
            </div>
        </div>
        <div class="table-body">
            ...
            <el-table
                        ...
                        <template v-else>
                            <div flex="dir:left cross:center" v-if="scope.row.condition_type == 1">
                                ...
                                <div style="margin-left: 5px;">
                                    <com-ellipsis :line="1">达到{{scope.row.condition}}人</com-ellipsis>
                                </div>
                            </div>
                            <div  flex="dir:left cross:center" v-if="scope.row.condition_type == 2">
                                <el-tag>{{condition_type_list[scope.row.condition_type]}}</el-tag>
                                <div style="margin-left: 5px;">
                                    <com-ellipsis :line="1">达到￥{{scope.row.condition}}</com-ellipsis>
                                </div>
                            </div>
                ...
                <el-table-column label="一级佣金">
                    <template slot-scope="scope">
                        <com-ellipsis :line="1" v-if="scope.row.first >= 0">{{scope.row.first}}{{price_type_list[scope.row.price_type]}}</com-ellipsis>
                        <template v-else>-</template>
                    </template>
                </el-table-column>
                <el-table-column label="二级佣金">
                    <template slot-scope="scope">
                        <com-ellipsis :line="1" v-if="scope.row.second >= 0">{{scope.row.second}}{{price_type_list[scope.row.price_type]}}</com-ellipsis>
                        <template v-else>-</template>
                    </template>
                </el-table-column>
                <el-table-column label="三级佣金">
                    <template slot-scope="scope">
                        <com-ellipsis :line="1" v-if="scope.row.third >= 0">{{scope.row.third}}{{price_type_list[scope.row.price_type]}}</com-ellipsis>
                        <template v-else>-</template>
                    </template>
                </el-table-column>
                <el-table-column
    ...
    const app = new Vue({
        ...
        methods: {
            ...
            edit(id) {
                if (id) {
                    navigateTo({
                        r: 'mall/distribution/level-edit',
                        id: id,
                    });
                } else {
                    navigateTo({
            ...
            switchStatus(row) {
                ...
                request({
                    params: {
                        r: 'mall/distribution/switch-status',
                    },
                    method: 'post',
                    data: {
                        id: row.id,
            ...
            destroy(row, index) {
                ...
                self.$confirm('删除该分销商等级, 是否继续?', '提示', {
                    ...
                    request({
                        params: {
                            r: 'mall/distribution/level-destroy',
                        },
                        method: 'post',
                        data: {
                            id: row.id,
