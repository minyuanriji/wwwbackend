...
class OrderPriceForm extends BaseModel
{
    public $order_id;
    ...
    public function updateExpress()
    {
        ...
        try {
            $this->mch_id = $this->mch_id ?: \Yii::$app->admin->identity->mch_id;
            $order = Order::findOne([
                'id' => $this->order_id,
                'is_delete' => 0,
    ...
    public function updatePrice()
    {
        ...
        try {
            ...
            if ($this->mch_id > 0) {
                ...
                $shareOrder = DistributionOrder::findOne(['order_detail_id' => $orderDetail->id, 'is_delete' => 0]);
                $totalSharePrice = 0;
                if ($shareOrder) {
...
//                ])->scalar();
    ...
    public function updateTotalPrice()
    {
        ...
        try {
            ...
            if ($this->mch_id > 0) {
                // 多商户修改单价金额 不能低于总的分销金额 + 商户佣金
                /* @var DistributionOrder[] $shareOrder */
                $shareOrder = DistributionOrder::find()->where(['order_id' => $order->id, 'is_delete' => 0])->all();
                $totalSharePrice = 0;
                if ($shareOrder) {
