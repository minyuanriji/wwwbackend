<?php
echo $this->render('../components/refund/com-remark');
echo $this->render('../components/refund/com-agree-refund');
?>
...
<style>
    ...
    .com-order-status .el-step__icon.is-text {
        border: 0px;
        width: 40px;
    }
        ...
        <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
            ...
            <div slot="header">
                <el-breadcrumb separator="/">
                    <el-breadcrumb-item><span style="color: #409EFF;cursor: pointer"
                                              @click="$navigate({r:'mch/order/refund'})">售后订单</span>
                    </el-breadcrumb-item>
            ...
            <div class="table-body" v-loading="loading">
                        ...
                        <div flex="dir:top" class="card-box">
                            ...
                            <template v-if="order.status == 2">
                                <div v-if="order.is_confirm == 1 && order.type == 2" class="list-item-box">
                                    <div class="item-box" flex="dir:left cross:center">
                                        ...
                                        <div class="label">{{order.confirm_at}}</div>
                                    </div>
                                    <div>卖家发货</div>
                                </div>
                            </template>
                            ...
                            <template v-if="order.status == 2">
                                <div v-if="order.is_confirm == 1 && order.type == 1 && (order.is_refund == 1 || order.is_refund == 2)"
                                    ...
                                    <div class="item-box" flex="dir:left cross:center">
                                        <span class="label">卖家:</span>
                                        <div v-if="order.is_refund == 1" class="label">{{order.refund_at|dateTimeFormat('Y-m-d H:i:s')}}</div>
                                        <div v-if="order.is_refund == 2" class="label">{{order.confirm_at|dateTimeFormat('Y-m-d H:i:s')}}</div>
                                    </div>
                                    <div>退款给买家</div>
                                </div>
                                <div v-if="order.is_confirm == 1 && order.type == 1" class="list-item-box">
                                    <div class="item-box" flex="dir:left cross:center">
                                        <span class="label">卖家:</span>
                                        <div class="label">{{order.confirm_at|dateTimeFormat('Y-m-d H:i:s')}}</div>
                                    </div>
                                    <div>确认收货</div>
                                </div>
                            </template>
                            <div v-if="order.status == 2 && order.is_send == 1" class="list-item-box">
                                <div class="item-box" flex="dir:left cross:center">
                                    ...
                                    <div class="label">{{order.send_at|dateTimeFormat('Y-m-d H:i:s')}}</div>
                                </div>
                                <div>买家已发货</div>
                            </div>
                            <div v-if="order.status != 1" class="list-item-box">
                                <div class="item-box" flex="dir:left cross:center">
                                    <span class="label">卖家:</span>
                                    <div class="label">{{order.status_at|dateTimeFormat('Y-m-d H:i:s')}}</div>
                                </div>
                ...
                <el-card shadow="never" style="margin-top: 15px;">
                    ...
                    <el-form label-width="100px" :model="order" class="com-order-count-price">
                ...
                <div class="action-box" flex="dir:right">
                    <div>
                        ...
                        <el-button v-if="order.status == 2 && order.type == 1 && order.is_send == 1 && order.is_confirm == 0" size="small"
                        ...
                        <el-button
                                v-if="(order.type == 1 && order.status == 2 && order.is_send == 1 && order.is_confirm == 1 && order.is_refund == 0) ||
                                      (order.type == 0 && order.status == 2 && order.is_receipt == 0)"
            ...
            setStepActive() {
                ...
                if (this.order.is_confirm == 1 && this.order.status == 2) {
                    if (this.order.type == 1 && (this.order.is_refund == 1 || this.order.is_refund == 2)) {
                        ...
                        this.steps[2].description = this.order.is_refund == 2 ? this.order.confirm_at : this.order.refund_time;
                    }
                    if (this.order.type == 2 && (this.order.is_confirm == 1 || this.order.is_refund == 2)) {
                        this.stepActive = 3;
                        this.steps[2].title = '已完成售后';
                        this.steps[2].icon = 'statics/img/mall/order/refund_status/status_3_active.png';
                        this.steps[2].description = this.order.confirm_at;
                    }
                }
            },
            // 新的
