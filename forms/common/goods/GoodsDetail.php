...
class GoodsDetail extends BaseModel
{
    ...
    {
        $goods = Goods::find()->with(['attr', 'services', 'goodsWarehouse.cats', 'attr.memberPrice'])
            ->where([
                'is_delete' => 0,
                'id' => $id,
    ...
    public function getGoodsMarketingAward()
    {
        ...
        if ($this->goods->give_score > 0) {
            if ($this->goods->give_score_type == 1) {
                $title = sprintf('购买可得%s积分', $this->goods->give_score);
            } else {
                $price = max(array_column($this->goods->attr, 'price'));
                $title = sprintf('购买最高可得%s积分', bcmul($price, $this->goods->give_score / 100, 0));
            }
        } else {
            $title = '';
        }
    ...
    public function getAll($params = [], $ignore = [])
    {
        ...
        // 商品分销价格关闭
        $mallSetting = $this->mall->getMallSetting(['is_distribution_price', 'is_common_user_member_price', 'is_member_user_member_price', 'is_sales', 'is_express']);
        ...
        if ($this->goods->individual_share == 1) {
            ...
            if ($this->goods->attr_setting_type == 1) {
                foreach ($this->goods->attr as $item) {
                    $first = max($first, $item->share->distribution_commission_first);
                }
            } else {
                foreach ($this->goods->share as $item) {
                    if ($item->goods_attr_id == 0) {
                        $first = $item->distribution_commission_first;
                        break;
                    }
                }
            }
            ...
            $shareType = $shareSetting[DistributionSetting::PRICE_TYPE] == 2 ? 0 : 1;
            $first = $shareSetting[DistributionSetting::FIRST];
        }
        return [
            'first' => $first,
            'shareType' => $shareType,
