...
//            $val['individual_share'] = $val['individual_share'] == 1 ? '是' : '否';
            ...
            if(!empty($val['is_order_paid'])){
                ...
                $order_paid_msg = '积分：' . (empty($order_paid['is_score']) ? '关闭，' : '开启，') . '积分劵：' . (empty($order_paid['is_score_card']) ? '关闭，' : '开启，') . '金豆券：' . (empty($order_paid['is_integral_card']) ? '关闭。' : '开启。');
...
//            echo '<pre>';
    ...
    public function ExportGoodsData($arrData){
        ...
        $title = ['编号', '商品名称', '是否上架', '售价', '使用规格组', '规格组', '商品库存', '已出售量', '购物数量限制', '单品满件包邮', '单口满额包邮', '运费模板ID', '赠送积分类型', '可抵扣积分', '可抵扣积分类型', '允许多件累计折扣', '是否单独分销设置', '分销设置类型', '分类佣金类型', '默认服务', '使用积分', '金豆券可抵扣分', '是否启用金豆券赠送', '金豆券赠送设置', '是否启用积分券赠送', '积分券赠送设置','订单支付设置','订单支付参数','是否支持退换货','销量是否开启','虚拟销量是否开启','原价', '区域服务商','联合创始人','城市服务商'
        ];
