...
use app\notification\jobs\MchCashNotificationWeTplJob;
use app\notification\wechat_template_message\MchCashNotificationWeTplMsg;
use app\plugins\mch\models\Mch;
use app\plugins\mch\models\MchCash;

class MchCashNotification
{

    public static function send(MchCash $mch_cash)
    {
        (new MchCashNotificationWeTplJob([
            "mch_cash" => $mch_cash
        ]))->execute(null);

        /*\Yii::$app->queue->delay(0)->push(new MchCashNotificationWeTplJob([
            "mch_cash" => $mch_cash
        ]));*/
    }
    ...
    public static function sendWechatTemplate(MchCash $mch_cash)
    {
        ...
        $data = [];
        $template_id = '';
        if ($mch_cash->transfer_status == MchCash::TRANSFER_STATUS_ONE) {
        ...
        } elseif ($mch_cash->transfer_status == MchCash::TRANSFER_STATUS_TWO || $mch_cash->status == MchCash::STATUS_TWO) {
            $template_id = TemConfig::WithdrawalFailure;
            $data = [
                'first'     => '您的提现申请失败，资金已原路退回！',
                'keyword1'  => $mch_cash->money,
