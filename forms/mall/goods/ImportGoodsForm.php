...
class ImportGoodsForm extends BaseModel
{
    public $cat_ids;
    ...
    public function save()
    {
        ...
        try {
            ...
            if ($this->current_num <= 1) {
                ...
                $importGoods->mch_id = \Yii::$app->admin->identity->mch_id;
                $importGoods->user_id = \Yii::$app->admin->id;
            ...
            if (!$option) {
                $option = OptionLogic::set(Option::NAME_IMPORT_ERROR_LOG, $newArr, \Yii::$app->mall->id, Option::GROUP_ADMIN);
            }
            // 追加 错误数据
            ...
            if (!$res) {
                throw new \Exception($this->responseErrorMsg($importGoods));
            }
        ...
        try {
            ...
            while ($file_data = fgetcsv($cvs_file)) {
                ...
                if ($file_data[0] != '') {
                    ...
                    $arr['give_score'] = $this->getNewData($file_data[17]);
                    $arr['give_score_type'] = $this->getNewData($file_data[18]);
                    $arr['forehead_score'] = $this->getNewData($file_data[19]);
                    $arr['forehead_score_type'] = $this->getNewData($file_data[20]);
    ...
    {
        // 解决乱码
        return eval('return ' . iconv('gbk', 'utf-8', var_export($data, true)) . ';');
    }
    ...
    private function saveGoods($data)
    {
        ...
        try {
            ...
            if (!$goodsWarehouse->save()) {
                throw new \Exception($this->responseErrorMsg($goodsWarehouse));
            }
            $goods = new Goods();
            $goods->mall_id = \Yii::$app->mall->id;
            $goods->mch_id = \Yii::$app->admin->identity->mch_id;
            ...
            $option = OptionLogic::get(Option::NAME_MCH_MALL_SETTING, \Yii::$app->mall->id, Option::GROUP_APP);
            ...
            $goods->give_score = $data['give_score'];
            $goods->give_score_type = $data['give_score_type'];
            $goods->forehead_score = $data['forehead_score'];
            $goods->forehead_score_type = $data['forehead_score_type'];
            ...
            $goods->sign = \Yii::$app->admin->identity->mch_id > 0 ? 'mch' : '';
            ...
            if (!$mallGoods->save()) {
                throw new \Exception($this->responseErrorMsg($mallGoods));
            }

            if (\Yii::$app->admin->identity->mch_id > 0) {
                $mchGoods = new MchGoods();
                $mchGoods->mall_id = \Yii::$app->mall->id;
                $mchGoods->mch_id = \Yii::$app->admin->identity->mch_id;
