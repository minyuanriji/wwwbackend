...
class InitDataForm extends BaseModel
{
        ...
        for ($i = 0; $i <= $count; $i += $limit) {
            ...
            foreach ($list as $order) {
                foreach ($order->detail as $detail) {
                    ...
                    $goodsList[$detail['goods_id']]['payment_people'] += 1;
                    $goodsList[$detail['goods_id']]['payment_order'] += 1;
            ...
            foreach ($goodsList as $id => $goods) {
                $sql .= sprintf("WHEN %d THEN %d ", $id, $goods['payment_people']);
            }
            $sql .= "END, `payment_order` = CASE `id` ";
            foreach ($goodsList as $id => $goods) {
                $sql .= sprintf("WHEN %d THEN %d ", $id, $goods['payment_order']);
            }
