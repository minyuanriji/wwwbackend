...
/**
 * This is the model class for table "{{%plugin_stock_agent}}".
 *
 * @property int $id
 ...
 */
class StockAgent extends BaseActiveRecord
{
    ...
    {
        return $this->hasOne(StockLevel::class, ['level' => 'level','is_delete' => self::NO]);
    }
    ...
    public function afterSave($insert, $changedAttributes)
    {
        if (!$this->is_delete) {
            //新纪录才去
            if ($insert) {
                Yii::$app->trigger(StockInsertHandler::AGENT_INSERT,
                    new StockInsertEvent(['user_id' => $this->user_id, 'mall_id' => $this->mall_id]));
            } else {
                Yii::warning('不是新增分销商');
            }
        }
