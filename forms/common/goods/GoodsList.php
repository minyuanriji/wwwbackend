...
class GoodsList extends BaseModel
{
    ...
    public function rules()
    {
        return [
            [['limit', 'is_negotiable', 'status', 'sort', 'sort_type', 'is_sell_well',
                'mch_id', 'is_cart', 'is_sold_out', 'mch_status', 'is_sales'], 'integer'],
    ...
    private function idsWhere()
    {
        if(count($this->ids)>0){
            $this->query->andWhere(['in', 'g.id', $this->ids]);
        }
    }
    ...
    public function getDiyBack($goods)
    {
        ...
        $price_display       = $PriceDisplayService->getGoodsPriceDisplay($goods->price_display);
        ...
        $newItem = [
            ...
            'price_display'       => !empty($goods->price_display) ? $price_display : []
        ];
        return $newItem;
    }
