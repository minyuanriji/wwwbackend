...
Yii::$app->loadPluginComponentView('distribution-batch');
Yii::$app->loadPluginComponentView('distribution-edit');
Yii::$app->loadPluginComponentView('distribution-level');
?>
...
<div id="app" v-cloak>
        ...
        <div class="table-body">
            ...
            <el-select size="small" v-model="search.level" @change='toSearch' class="select">
                ...
                <el-option :key="index" :label="item.name" :value="item.level"
                           v-for="(item, index) in distributionLevelList"></el-option>
            </el-select>
            ...
            <div class="batch">
                <distribution-batch :choose-list="choose_list" :distribution-level-list="distributionLevelList"
                           @to-search="loadData"></distribution-batch>
            </div>
            <div style="float: right">
                <el-button type="primary" size="small" style="padding: 9px 15px !important;" @click="editClick">添加分销商
                </el-button>
            </div>
            <el-tabs v-model="activeName" @tab-click="handleClick">
                <el-table :data="list" border v-loading="loading" size="small" style="margin-bottom: 15px;"
                    ...
                    <el-table-column label="时间" width="200">
                        <template slot-scope="scope">
                            <div>成为分销商时间：<br>{{scope.row.created_at|dateTimeFormat('Y-m-d H:i:s')}}</div>
                        </template>
                    </el-table-column>
                    </el-table-column>
                        ...
                        <template slot-scope="scope">
                            <el-button type="text" size="mini" circle style="margin-top: 10px"
                                ...
                                <el-tooltip class="item" effect="dark" content="查看订单" placement="top">
                                    <img src="statics/img/mall/distribution/order.png" alt="">
                                </el-tooltip>
                            </el-button>
                            <el-button type="text" size="mini" circle style="margin-left: 10px;margin-top: 10px"
                                ...
                                <el-tooltip class="item" effect="dark" content="提现详情" placement="top">
                                    <img src="statics/img/mall/distribution/detail.png" alt="">
                                </el-tooltip>
                            </el-button>
                            <el-button type="text" size="mini" circle style="margin-left: 10px;margin-top: 10px"
    ...
    <distribution-edit v-model="edit.show"></distribution-edit>
    <distribution-level v-model="level.show" :distribution="level.distribution" @success="levelSuccess"></distribution-level>
</div>
<script>
    const app = new Vue({
        el: '#app',
        ...
        methods: {
            agentDelete(row, index) {
                ...
                self.$confirm('删除该分销商, 是否继续?', '提示', {
                    ...
                    request({
                        params: {
                            r: 'plugin/distribution/mall/distribution/delete',
                        },
                        method: 'post',
                        data: {
                            id: row.id,
            ...
            loadData() {
                this.loading = true;
                let params = {
                    r: 'plugin/distribution/mall/distribution/index'
                };
                ...
                request({
                    ...
                    if (e.data.code == 0) {
                        ...
                        this.distributionLevelList = e.data.data.distributionLevelList;
                    } else {
            ...
            remarksSubmit() {
                ...
                request({
                    params: {
                        r: 'plugin/distribution/mall/distribution/remarks-edit',
                    },
                    method: 'post',
                    data:{
