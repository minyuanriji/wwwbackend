...
class SubmitOrderAction extends BaseObject
{
     ...
     *  szAgentId	String(16)	是	客户ID(需联系商务生成)
        ...
        szPhoneNum	String(32)	是	充值号码、QQ号、加油卡号等
        nMoney	Integer	是	充值金额  话费产品单位为：元  流量产品单位为：M  视频会员：填写产品原价
        nSortType	Integer	是	运营商编码：见附录2.1
        nProductClass	Integer	是	固定值：1
        nProductType	String	是	固定值：1
        szProductId	String	否	产品编码  常规话费不需要填写（如需指定产品编码需与商务沟通确认） 特定产品必填如流量产品
        szTimeStamp	DateTime	是	时间戳。用于判断过期响应，与接口服务器时间间隔1分钟内有效。格式:yyyy-MM-dd HH:mm:ss(例如：2016-01-01 07:23:00)
        szVerifyString	String	是	签名
        szNotifyUrl	String	是	完成结果回调通知地址（不参与签名）
        szFormat	String	否	结果返回格式:JSON（默认）  XML
        ...
        try {
            ...
            if ($parseArray['nRtn'] != Code::ORDER_SUCCESS) {
                if (isset($parseArray['szRtnCode'])) {
                    throw new \Exception(Msg::submitMsg()[$parseArray['nRtn']], ApiCode::CODE_FAIL);
                } else {
