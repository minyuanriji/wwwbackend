...
class OrderHelper
{
     ...
     */
    public static function submitPlateformOrder(AddcreditOrder $orderModel, AddcreditPlateforms $plateform)
    {
        try {
            $className = '\\app\\plugins\\addcredit\\plateform\\sdk\\' . $plateform->sdk_dir . '\\PlateForm';
            if (empty($className) || !class_exists($className)) {
                throw new \Exception("缺失平台类文件", ApiCode::CODE_FAIL);
            }
            $classObject = new $className();
            if (!$classObject instanceof IOrder) {
                throw new \Exception("平台类文件未实现IOrder接口", ApiCode::CODE_FAIL);
            }

            $result = $classObject->submit($orderModel);
            if (!$result instanceof SubmitResult) {
                throw new \Exception("结果对象返回类型[SubmitResult]错误", ApiCode::CODE_FAIL);
            }

            if ($result->code != SubmitResult::CODE_SUCC) {
            ...
            return [
                ...
                'data' => [
                    'plateform_order_no' => $result->order_no,
                ]
            ];
        } catch (\Exception $e) {
            return [
     ...
     * @param AddcreditPlateforms $plateform
     */
    public static function queryPlateformOrder(AddcreditOrder $orderModel, AddcreditPlateforms $plateform)
    {
        try {
            $className = '\\app\\plugins\\addcredit\\plateform\\sdk\\' . $plateform->sdk_dir . '\\PlateForm';
            if (empty($className) || !class_exists($className)) {
            ...
            $result = $classObject->query($orderModel);
            if (!$result instanceof QueryResult) {
                throw new \Exception("结果对象返回类型[QueryResult]错误", ApiCode::CODE_FAIL);
            }
            ...
            if (in_array($result->status, ['waiting','success','fail'])) {
                throw new \Exception("查询订单状态错误！", ApiCode::CODE_FAIL);
            }

            return [
                'code' => ApiCode::CODE_SUCCESS,
                'data' => [
                    "plateform_order_no" => $orderModel->order_no,
