...
class SendForm extends BaseModel
{
    public $name;
    public $order;
    ...
    protected function coupon_where()
    {
        $query = UserCoupon::find()->alias('uc')
            ->select("DATE_FORMAT(uc.`created_at`,'%Y-%m-%d') AS `date`,c.`name`,
                                ...
                                SUM(CASE WHEN uc.`end_at`< NOW() AND uc.`is_use` = 0 THEN 1 ELSE 0 END) AS `end_num`")
            ->leftJoin(['c' => Coupon::tableName()], 'c.id = uc.coupon_id')
            ->leftJoin(['i' => User::tableName()], 'i.id = uc.user_id')
        ...
        //时间查询
        if ($this->date_start) {
            $query->andWhere(['>=', 'uc.created_at', strtotime($this->date_start . ' 00:00:00')]);
        }

        if ($this->date_end) {
            $query->andWhere(['<=', 'uc.created_at', strtotime($this->date_end . ' 23:59:59')]);
        }
        //平台标识查询
        if ($this->platform) {
