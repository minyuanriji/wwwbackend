...
<!--        </el-breadcrumb>-->
        ...
        <div flex="cross:center box:first">
            <div><span @click="$navigate({r:'mall/card/index'})" class="text">优惠券管理</span> / 优惠券发放</div>
            <div flex="dir:right">
                <div>
                    <el-button class="button-item" type="primary" size="small" :loading=submitLoading @click="onSubmit">保存</el-button>
                </div>
            </div>
        </div>
    </div>
                ...
                <el-tab-pane label="基础设置" name="first">
                    <div class="form-body">
                        ...
                        <el-form-item label="优惠券类型" prop="type">
                            <el-radio v-model="form.type" label="2">满减券</el-radio>
                            <el-radio v-model="form.type" label="1">打折券</el-radio>
                        </el-form-item>
                            ...
                            <template v-else>
                                <div flex>
                                    <div style="color: #ff4544;font-size: 12px;margin-left: 24px;">注：必须在“
                                        <el-button type="text"
                                                   @click="$navigate({r:'mall/member-level/index'}, true)">
                                            用户管理=>会员等级
                                        </el-button>
                                        ”中设置了会员，才能使用
                                    </div>
    ...
    const app = new Vue({
        ...
        methods: {
            ...
            onSubmit() {
                ...
                this.form.begin_at = this.time[0];
                this.form.end_at = this.time[1];
                ...
                request({
                    ...
                    if (e.data.code == 0) {
                        if (e.data.data.list && e.data.data.list.id > 0) {
                            ...
                            this.time = [e.data.data.list.begin_at, e.data.data.list.end_at];
                            if (e.data.data.list.begin_at == '0000-00-00 00:00:00' &&  e.data.data.list.end_at == '0000-00-00 00:00:00') {
                                this.time = [];
                            }
