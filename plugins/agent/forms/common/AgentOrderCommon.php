...
use app\models\User;
use app\plugins\agent\events\AgentCommissionEvent;
use app\plugins\agent\handlers\AgentCommissionHandler;
...
class AgentOrderCommon extends BaseModel
{
    ...
    public static function addAgentOrder($order,$level){
        ...
        try{
            //是否开启分销内购
            $agentSetting = AgentSetting::getValueByKey(AgentSetting::IS_SELF_BUY);
            //判断下单人自己是否分销商
            ...
            $agent = isset($user->agent) ? $user->agent : [];
            //$agent = Agent::find()->where(['mall_id' => $order->mall_id, 'user_id' => $user_id, 'is_delete' => 0])->with("user")->one();
            if ((!empty($agentSetting) && $agentSetting == 1) && !empty($agent)) {
                ...
                //没有满足分销内购条件，则按级别进行分销
                $firstParentUser = Agent::findOne(['mall_id' => $order->mall_id,'user_id' => $user->parent_id, 'is_delete' => 0]);
            }
            ...
            //分销设置
            $agentSetting = AgentSetting::getData($order->mall_id);
            //分销等级
            if($level >= 2){
                if($second_user_id > 0){
                    $secondParentUser = Agent::findOne(['mall_id' => $order->mall_id,'user_id' => $second_user_id, 'is_delete' => 0]);
                }
                if($third_user_id > 0){
                    $thirdParentUser = Agent::findOne(['mall_id' => $order->mall_id,'user_id' => $third_user_id, 'is_delete' => 0]);
                }
            }
            //订单详情
            ...
            foreach ($orderDetails as $orderDetail) {
                ...
                if ((isset($agentGoodsInfo['is_alone']) && $agentGoodsInfo['is_alone'] == 1)) {
                    ...
                    // 全局设置
                    $shareType = $agentSetting[AgentSetting::PRICE_TYPE] == 2 ? 0 : 1;
                    if ($firstParentUser) {
                        if ($firstParentUser->level > 0) {
                            $first_price = self::getAgentLevelGlobal($firstParentUser, $orderDetail, AgentSetting::FIRST_PRICE);
                        } else {
                            if (!empty($agentSetting[AgentSetting::FIRST_PRICE])
                                && is_numeric($agentSetting[AgentSetting::FIRST_PRICE])) {
                                $first_price = $agentSetting[AgentSetting::FIRST_PRICE];
                                $first_price = self::getAgentPrice($first_price, $shareType, $orderDetail);
                            }
                        }
                    }
                    ...
                    if ($secondParentUser) {
                        if ($secondParentUser->level > 0) {
                            $second_price = self::getAgentLevelGlobal($secondParentUser, $orderDetail, AgentSetting::SECOND_PRICE);
                        } else {
                            if (!empty($agentSetting[AgentSetting::SECOND_PRICE])
                                && is_numeric($agentSetting[AgentSetting::SECOND_PRICE])) {
                                $second_price = $agentSetting[AgentSetting::SECOND_PRICE];
                                $second_price = self::getAgentPrice($second_price, $shareType, $orderDetail);
                            }
                        }
                    }
                    ...
                    if ($thirdParentUser) {
                        if ($thirdParentUser->level > 0) {
                            $third_price = self::getAgentLevelGlobal($thirdParentUser, $orderDetail, AgentSetting::THIRD_PRICE);
                        } else {
                            if (!empty($agentSetting[AgentSetting::THIRD_PRICE])
                                && is_numeric($agentSetting[AgentSetting::THIRD_PRICE])) {
                                $third_price = $agentSetting[AgentSetting::THIRD_PRICE];
                                $third_price = self::getAgentPrice($third_price, $shareType, $orderDetail);
                            }
                        }
                    }
    ...
    {
        $price = 0;
        $goods_agent_level = AgentLevel::find()->where(["is_delete" => 0,"mall_id" => \Yii::$app->mall->id])->asArray()->all();
    ...
    {
        $agentLevel = AgentLevelCommon::getInstance()->getAgentLevelByLevel($agent->level);
        if ($agentLevel) {
    ...
    public static function updateAgentOrder($order,$type,$order_detail_id = 0){
        ...
        try{
            //退款
            if($type == 2){
                $updateCondition = ["is_refund" => AgentOrder::YES];
                $fields = ["order_detail_id" => $order_detail_id];
                $result = AgentOrder::updateAll($updateCondition, $fields);
                if($result === false){
                    throw new \Exception('分销订单保存出错');
                }
                //分销订单
                $agentOrders = AgentOrder::findOne([
                    'mall_id' => $order->mall_id,
                    'order_id' => $order->id,
                ...
                $flag = "transfer";
                $result = AgentOrder::updateAll(['is_transfer' => 1], [
                    'mall_id' => $order->mall_id, 'order_id' => $order->id, 'is_delete' => 0
                ]);
                if($result === false){
                    throw new \Exception('分销订单保存出错');
                }
                $agentOrderList = AgentOrder::findAll([
                    'mall_id' => $order->mall_id, 'order_id' => $order->id, 'is_delete' => 0
                ]);
            }else if($type == 4){//更新支付状态
                $flag = "update_is_pay";
                $result = AgentOrder::updateAll(['is_pay' => 1], [
                    'mall_id' => $order->mall_id, 'order_id' => $order->id, 'is_delete' => 0
                ]);
                if($result === false){
    ...
    public static function getAgentGoodsInfo($goods_id,$goods_info){
        ...
        /** @var AgentGoods $agentGoodsInfo */
        $agentGoodsInfo = AgentGoods::find()->where(["goods_id" => $goods_id,"is_delete" => AgentGoods::NO,
            'goods_type' => AgentGoods::TYPE_MALL_GOODS])->one();
        if(!empty($agentGoodsInfo)){
            $returnData["is_alone"] = $agentGoodsInfo->is_alone;
            //是否单独设置
            if($agentGoodsInfo->is_alone == AgentGoods::YES){
                //0普通设置（按商品）1详细设置（按商品规格）
                if($agentGoodsInfo->attr_setting_type == AgentGoods::ATTR_SETTING_TYPE_GOODS){
                    \Yii::warning("分销按商品");
                    $agentGoodsDetail = AgentGoodsDetail::find()->select([
                        'commission_first', 'commission_second', 'commission_third', 'level'
                    ])->where(['goods_id' => $goods_id, 'goods_attr_id' => 0, 'is_delete' => 0, 'goods_type' => AgentGoodsDetail::TYPE_MALL_GOODS])->one();
                }else{
                    $attr_id = isset($goods_info["goods_attr"]["id"]) ? $goods_info["goods_attr"]["id"] : 0;
                    \Yii::warning("分销按商品规格 商品分类id:".$attr_id);
                    $agentGoodsDetail = AgentGoodsDetail::find()->select([
                        'commission_first', 'commission_second', 'commission_third', 'level'
                    ])->where(['goods_id' => $goods_id, 'goods_attr_id' => $attr_id, 'goods_type' => AgentGoodsDetail::TYPE_MALL_GOODS])->one();
                }
