 ...
 * @property User $user
 */
class CouponAutoSendCommon extends Model
{
    ...
    public function send()
    {
        ...
        foreach ($auto as $item) {
            ...
            $commonCoupon = new CouponCommon();
            $commonCoupon->mall = $this->mall;
            $commonCoupon->user = $this->user;

            $commonUserCouponAuto = new UserCouponAutoCommon($coupon, $item);
            if ($commonCoupon->receive($coupon, $commonUserCouponAuto, '自动发放优惠券')) {
                ...
                if ($coupon['expire_type'] == 1) {
                    ...
                    $coupon['desc'] = "本券有效期" . $coupon['begin_time'] . "至" . $coupon['end_at'];
                }
                $coupon['content'] = '限';
