<template id="agent-level">
    <el-dialog :visible.sync="edit.visible" width="20%" title="修改经销商等级">
        <div v-loading="edit.loading">
            <el-form size="small" label-width="150px">
                <el-form-item label="等级名称">
                    <el-select size="small" v-model="edit.level" class="select">
                        <el-option :key="index" :label="item.name" :value="item.level" :disabled="agent.level == item.level"
    ...
    Vue.component('agent-level', {
        ...
        watch: {
            value() {
                if (this.value) {
                    ...
                    this.edit.level = this.agent.level;
                    this.getLevel();
                } else {
                    this.edit.id = '';
        ...
        methods: {
            ...
            editSave() {
                ...
                request({
                    params: {
                        r: 'plugin/agent/mall/agent/level-change',
                    },
                    method: 'post',
                    data: {
                        level: this.edit.level,
                        id: this.agent.user_id
                    }
                }).then(response => {
