...
namespace app\mch\forms\statistics;
...
class DataForm extends BaseModel{
    ...
    public function search($type = 0){
        ...
        if ($type == 1 || $type == 0) {
            ...
            $goods_top_list = $goods_query
                ...
                ->asArray()
                ->all();
            $goods_top_list = Order::getGoodsName($goods_top_list);
        }
  ...
  COUNT(DATE_FORMAT(`o`.`created_at`, '%Y-%m-%d')) AS `order_num`,SUM(`o`.`total_pay_price`) AS `total_pay_price`,SUM(`d`.`num`) AS `goods_num`")
    ...
    public function all_search()
    {
        ...
        $all_data = $this->get_all_data();
        //插件分类订单list
        $plugins_list = Order::find()->select('sign')->where(['and', ['in', 'sign', ['appointing', 'score_mall', 'seckill', 'spell_group', 'bargain']], 'mall_id' => \Yii::$app->mall->id,])->groupBy('sign')->asArray()->all();
    ...
    protected function get_all_data(){
        ...
        /*$data_arr['user_count'] = $user_query->andWhere(['u.mch_id' => 0, 'u.is_delete' => 0, 'u.mall_id' => \Yii::$app->mall->id,])
            ->count();*///用户数
        $data_arr['user_count'] = StatisticsVirtualConfig::find()->where(['mall_id'=>\Yii::$app->mall->id,'is_delete'=>0])->select([ 'user_sum'])->asArray()->one();
        //以下随时间查询改变
  ...
  COUNT(DATE_FORMAT(`o`.`created_at`, '%Y-%m-%d')) AS `order_num`,SUM(`o`.`total_pay_price`) AS `total_pay_price`,SUM(`d`.`num`) AS `goods_num`");
        ...
        $query->groupBy('time')->orderBy('time');
    ...
    protected function table_where($bool = false){
        ...
        if ($this->date_start && $bool) {
            ...
            $query->andWhere(['>=', 'o.created_at', strtotime(date("Y-m-d", strtotime("-1 day")) . ' 00:00:00')]);
        }
        if ($this->date_end && $bool) {
            $query->andWhere(['<=', 'o.created_at', strtotime($this->date_end . ' 23:59:59')]);
        } else {
            $query->andWhere(['<=', 'o.created_at', strtotime(date("Y-m-d", strtotime("-1 day")) . ' 23:59:59')]);
        }
