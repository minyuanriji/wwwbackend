...
<div id="app" v-cloak>
    <el-card v-loading="loading" shadow="never" style="border:0"
             body-style="background-color: #f3f3f3;padding: 10px 0 10px 0px;">
        <div class="table-body" style="margin:0px 10px 10px 10px;">
            <com-search
                @to-search="toSearch"
                @search="searchList"
                :new-search="search"
            ...
            <el-card shadow="never">
                ...
                <el-table v-loading="user_loading" :row-style="{height:'63px'}" @sort-change="changeUser"
                    ...
                    <el-table-column prop="name" label="用户" width="400">
                        <template slot-scope="scope">
                            <com-image style="margin-right: 10px;float: left;" :src="scope.row.avatar" width="32px"
                                       height="32px">
                            </com-image>
    ...
    const app = new Vue({
        ...
        data() {
            return {
                ...
                userProp: '',
                goodsProp: '',
                user_url: '<?= $urlManager->createUrl('mch/overview/users_top')?>',
                goods_url: '<?= $urlManager->createUrl('mch/overview/goods_top')?>',
                is_mch_role: true
            };
        },
        methods: {
