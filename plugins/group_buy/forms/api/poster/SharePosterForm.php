    ...
    public function get()
    {
        ...
        if(\Yii::$app->appPlatform == User::PLATFORM_MP_WX){
            $file = $this->qrcode($option, [
                ['goods_id' =>  $this->goods_id,'is_share' => 1,'detailId' => $this->detail_id, 'pid' => \Yii::$app->user->id,'source'=>User::SOURCE_SHARE_GOODS,'mall_id'=>\Yii::$app->mall->id],
                240,
                '/mch/group-buy/detail'
            ], $this);
            isset($option['qr_code']) && $option['qr_code']['file_path'] = $file;
            isset($option['head']) && $option['head']['file_path'] = self::head($this);
        }else{
            $path = "/h5/#/mch/group-buy/detail?goods_id=" . $this->goods_id . "&is_share=1&detailId=" . $this->detail_id . "&mall_id=" . \Yii::$app->mall->id . "&pid=" . $user_id . "&source=" . User::SOURCE_SHARE_GOODS;
            $dir = 'group_buy/share/' . $this->goods_id . "and" . $this->detail_id . '.jpg';
            $file = CommonLogic::createQrcode($option, $this, $path, $dir);
            isset($option['qr_code']) && $option['qr_code']['file_path'] = $file;
            isset($option['head']) && $option['head']['file_path'] = self::head($this, "group_buy/share/");
        }
