...
class KpiAdminStatRegisterForm extends BaseModel{
    ...
    public function getList(){
        ...
        try {
            ...
            $query = KpiRegister::find()->alias("krs")
                ->innerJoin(["u" => User::tableName()], "u.id=krs.inviter_user_id")
                ->groupBy("krs.inviter_user_id");

            $query->where(["krs.merchant_id" => $this->merchant_id]);
            if($this->store_id){
                $query->where(["krs.store_id" => $this->store_id]);
            }
            ...
            $query->andWhere([">", "krs.created_at", $this->start_time]);
            $query->andWhere(["<", "krs.created_at", $this->end_time]);
            ...
            $selects[] = "count(krs.inviter_user_id) as num";
            $selects[] = "sum(krs.point) as total_point";
