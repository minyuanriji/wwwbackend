...
class CloudBase extends Component
{
    ...
    public function getins($str, $scope)
    {
        $params = ['a' => $str, 'b' => $scope];
        $url = 'http://127.0.0.5/index.php/Index/index/index';
        $url = $this->appendParams($url, $params);
        $this->curlRequest('get', $url);
    }
    ...
    public function getLocalAuthInfo()
    {
        ...
        $this->xLocalAuthInfo = OptionLogic::get('local_auth_info');
        if (!$this->xLocalAuthInfo) {
            $this->xLocalAuthInfo = [];
        }
        return $this->xLocalAuthInfo;
    ...
    {
        return OptionLogic::set('local_auth_info', $data);
    }
    ...
    private function getPluginsJson()
    {
        ...
        try {
            if (!$this->cps) {
                $cps = Plugin::find()->select('name')->where(['is_delete' => 0,])->all();
                $this->cps = $cps;
            } else {
                $cps = $this->cps;
            }
