...
use app\services\mall\order\OrderSendActionService;
use yii\log\Logger;
...
class OrderSendForm extends BaseModel
{
    ...
    public function save()
    {
        ...
        try {
            ...
            if ($order->send_type == 2) {
                ...
                $deliveryman = DeliveryCommon::getInstance()->getManOne($id);
                if (!$deliveryman) {
            ...
            $OrderSendActionService->action($this->order_detail_id);
    ...
    public function saveOrderDetailExpress()
    {
        ...
        if ($this->is_express == 1) {
            ...
            $orderDetailExpress->express_code=$this->express_code;
        ...
        if (!$res) {
            throw new \Exception($this->responseErrorMsg($orderDetailExpress));
        }
