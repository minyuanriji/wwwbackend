...
use app\forms\mall\export\ScoreStatisticsExport;
...
class ScoreForm extends BaseModel
{
    ...
    public function search()
    {
        ...
        $query->select(["DATE_FORMAT(`il`.`created_at`, '%Y-%m-%d') AS `date`,
                         COALESCE(SUM(CASE `il`.`type` WHEN 1 THEN `il`.`score` ELSE 0 END), 0) AS in_integral,
                         COALESCE(SUM(CASE `il`.`type` WHEN 2 THEN `il`.`score` ELSE 0 END), 0) AS out_integral"]);
    ...
    {
        $query = ScoreLog::find()->alias('il')->where(['il.mall_id' => \Yii::$app->mall->id,])
            ->leftJoin(['i' => User::tableName()], 'i.id = il.user_id');
