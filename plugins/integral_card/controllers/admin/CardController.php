...
use app\plugins\integral_card\models\Card;
use app\plugins\integral_card\models\CardDetail;
use app\plugins\integral_card\models\ProfitAgent;
use app\plugins\integral_card\models\ProfitCard;
use app\plugins\integral_card\models\ProfitCardDetail;
...
class CardController extends BaseController{
        ...
        if (\Yii::$app->request->isAjax) {
            if (\Yii::$app->request->isPost) {
                ...
                if($current_batch == 'copy'){
                    ...
                    if (!$agent) {
                        return $this->error('该用户不存在或者已被删除！');
                    }
                    $data = [
                        'mall_id' => $mall_id,
                        'user_id' => $new_user_id,
     ...
     * @return void
     */
    public function actionChangeAgentOne(){
        if (\Yii::$app->request->isAjax) {
            if (\Yii::$app->request->isPost) {
                $new_user_id = \Yii::$app->request->post('user_id');
                $card_id = \Yii::$app->request->post('card_id');
                $mall_id = \Yii::$app->mall->id;

                $card = Card::find()->where(['id' => $card_id, 'mall_id'=>$mall_id])->one();
                if($card){
                    $agent = User::findOne(['id' => $new_user_id, 'mall_id'=>$mall_id, 'is_delete' => 0]);
                    if (!$agent) {
                        return $this->error('该用户不存在或者已被删除！');
                    ...
                    $res = $card->save();
                    if($res){
                        $resx = CardDetail::updateAll(['user_id'=>$new_user_id],['card_id'=>$card_id,'mall_id'=>$mall_id]); 
                        if($resx === false) return $this->error(CardDetail::getError());
                        return $this->success('success');
                    }else{
                        return $this->error(Card::getError());
                    ...
                    return $this->error('该卡券不存在或者已被删除！');
                }
            }
        }
    }
    ...
    public function actionCardList(){
        ...
        if ($request->isAjax) {
            ...
            if($type == '1' && $keyword){
                $where[] = ['or',['like', 'u.nickname', $keyword],['like', 'u.mobile', $keyword]];
            }elseif($type == '2' && $keyword){
                $where[] = ['or',['like', 'p.nickname', $keyword],['like', 'p.mobile', $keyword]];
            }elseif($type == '3'){
                $where[] = array('=','c.serialize_no',$keyword);
            }
            if($status != 999)  $where[] = array('=','c.status',$status);
            $params = array(
                'alias' => 'c',
                'where' => $where,
                'joinWith' => ['user as u','picker as p'],
                'limit' => $this->pageSize,
                'order' => 'id ASC'
            );
            //获取数据
    ...
    public function actionExportCardInfo(){
        ...
        $csv = new CsvExport();
        $fileName = "积分卡券-".date('YmdHis', time());
