 ...
 */
abstract class GoodsEditModel extends BaseModel
{
    public $id;
    ...
    public function rules()
    {
        return [
            ...
            [['cats', 'mchCats', 'services', 'cards', 'attr', 'attrGroups', 'member_price',
                'select_attr_groups', 'distributionLevelList'], 'safe'],
    ...
    protected function setGoods()
    {
        ...
        if (!$res) {
            throw new Exception($this->responseErrorInfo($goods));
        }
    ...
    protected function setAttr()
    {
        ...
        $oldAttr = Attr::find()->where([
            'is_delete' => 0, 'goods_id' => $this->goods->id
        ])->select('id')->asArray()->all();

        // 是否为新增
        if (!$this->isNewRecord) {
            Attr::updateAll(['is_delete' => 1,], ['goods_id' => $this->goods->id, 'is_delete' => 0]);
            // GoodsShare::updateAll(['is_delete' => 1], ['is_delete' => 0, 'goods_id' => $this->goods->id]);
            MemberPrice::updateAll(['is_delete' => 1], ['goods_id' => $this->goods->id, 'is_delete' => 0]);
        }
        if ($this->attr_setting_type == 0) {
            // 普通设置
            // 默认分销价的商品规格ID为 0
            //$this->setGoodsDistribution(0, $this->distributionLevelList);
            // $this->checkMchDistributionPrice($this->distributionLevelList, $this->price, $this->share_type);
        }
        ...
        foreach ($this->newAttrs as $newAttr) {
            ...
            if (!$res) {
                throw new Exception($this->responseErrorInfo($goodsAttr));
            }
            ...
            if ($this->attr_setting_type == 1) {
                if ($this->use_attr == 1) {
                    // 详细设置
                    //    $this->setGoodsDistribution($goodsAttr->id, $newAttr['distributionLevelList']);
                    // $this->checkMchDistributionPrice($newAttr['distributionLevelList'], $newAttr['price'], $this->share_type);
                } else {
                    // 详细设置(不适用规格)
                    // $this->setGoodsDistribution($goodsAttr->id, $this->distributionLevelList);
                    //$this->checkMchDistributionPrice($this->distributionLevelList, $newAttr['price'], $this->share_type);
                }
            }
        ...
        $res = $this->goods->save();
        if (!$res) {
            throw new \Exception($this->responseErrorInfo($this->goods));
        }
    }
    ...
    /**
     * @param $goodsAttrId
     * @param array $distributionLevelList
     * @throws \Exception
     * @return boolean
     */
    protected function setGoodsDistribution($goodsAttrId, $distributionLevelList)
    {
        ...
        $list = [];
        if (!$this->isNewRecord) {
            $res = GoodsDistribution::find()
                ->where(['goods_id' => $this->goods->id, 'goods_attr_id' => $goodsAttrId, 'is_delete' => 0])
                ->all();
            /* @var GoodsDistribution[] $res */
            foreach ($res as $item) {
                $item->is_delete = 1;
                $list[$item->level] = $item;
            }
        }
        /* @var GoodsDistribution[] $list */
        foreach ($distributionLevelList as $shareLevel) {
            if (!isset($list[$shareLevel['level']])) {
                $goodsShare = new GoodsDistribution();
            ...
            $goodsShare->distribution_commission_first = $model->distribution_commission_first;
            $goodsShare->distribution_commission_second = $model->distribution_commission_second;
            $goodsShare->distribution_commission_third = $model->distribution_commission_third;
     ...
     * @throws \Exception
     */
    private function checkMchDistributionPrice($distributionLevelList, $goodsPrice, $shareType)
    {
        if (\Yii::$app->mchId) {
            if (empty($distributionLevelList)) {
                return;
            }
            $first = min(0, max(array_column($distributionLevelList, 'distribution_commission_first')));
            $second = min(0, max(array_column($distributionLevelList, 'distribution_commission_second')));
            $third = min(0, max(array_column($distributionLevelList, 'distribution_commission_third')));
            // 商城提成
            ...
            $moneyCount = $deductMoney + $shareMoney;
