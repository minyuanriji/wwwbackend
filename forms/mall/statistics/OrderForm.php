...
class OrderForm extends BaseModel
{
    ...
    public function search()
    {
        ...
        //时间查询
        if ($this->date_start) {
            $all_query->andWhere(['>=', 'o.created_at', strtotime($this->date_start . ' 00:00:00')]);
        }

        if ($this->date_end) {
            $all_query->andWhere(['<=', 'o.created_at', strtotime($this->date_end . ' 23:59:59')]);
        }
  ...
  COUNT(`o`.`id`) AS `order_num`,COALESCE(SUM(`o`.`total_pay_price`),0) AS `total_pay_price`,COALESCE(sum(`d`.`num`),0) as `goods_num`")
        ...
        $plugins_list = Order::find()->select('sign')
            ->where(['and', ['in', 'sign', ['appointing', 'score_mall', 'seckill', 'spell_group', 'bargain', 'advance','vip_card']], 'mall_id' => \Yii::$app->mall->id,])
            ->groupBy('sign')->asArray()->all();
        $permission = \Yii::$app->branch->childPermission(\Yii::$app->mall->admin->adminInfo);
    ...
    protected function where()
    {
        ...
        //订单状态分类查询
        $orderQuery->keyword($this->status == 3, ['od.refund_status' => [OrderDetail::REFUND_STATUS_SALES,OrderDetail::REFUND_STATUS_SALES_END_PAY]])
            ->keyword($this->status == 4, ['od.refund_status' => OrderDetail::REFUND_STATUS_SALES_END_PAY,OrderDetail::REFUND_STATUS_SALES_END_REJECT]);
