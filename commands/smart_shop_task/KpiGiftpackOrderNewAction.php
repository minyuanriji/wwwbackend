...
class KpiGiftpackOrderNewAction extends BaseTaskAction {
    ...
    public function whileRun() {
        ...
        $rows = $this->shop->getGiftpackOrders($selects, [
        ...
        foreach($rows as $row){
            $res = $kpi->newOrder("giftpack_order", $row['store_id'], $row['merchant_id'], $row['id'], $row['mobile'], $row['kpi_inviter_mobile']);
            if(!$res){
                $this->controller->commandOut($kpi->getError());
            }
        }

        $this->controller->commandOut("KPI用户新订单统计完成（giftpack_order）");
    }
}