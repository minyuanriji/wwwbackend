...
class AgentGoodsForm extends BaseModel
{
    public $goods_id;
    ...
    public function saveAgentGoodsSetting()
    {
        ...
        $agent_goods = AgentGoods::findOne(['goods_id' => $this->goods_id, 'is_delete' => 0, 'goods_type' => $this->goods_type]);
        if (!$agent_goods) {
            $agent_goods = new AgentGoods();
        ...
        foreach ($this->goods_agent_level_list as $item) {
            $agent_goods_detail = AgentGoodsDetail::findOne(['is_delete' => 0, 'agent_goods_id' => $agent_goods->id, 'level' => $item['level']]);
            if (!$agent_goods_detail) {
                $agent_goods_detail = new AgentGoodsDetail();
            ...
            $agent_goods_detail->agent_price = isset($item['agent']['agent_price']) ? $item['agent']['agent_price'] : 0;
    ...
    public function getAgentGoodsSetting()
    {
        ...
        $is_equal = AgentSetting::getValueByKey(AgentSetting::IS_ENABLE);
        if ($is_equal) {
            $setting['is_equal'] = $is_equal;
        }
        $level_list = AgentLevel::find()->where(['is_delete' => 0, 'mall_id' => \Yii::$app->mall->id])->orderBy('level ASC')->asArray()->all();
        if (!count($level_list)) {
            return [
                'code' => ApiCode::CODE_FAIL,
                'msg' => '请先添加经销商等级',
        ...
        $agent_goods = AgentGoods::findOne(['is_delete' => 0, 'goods_id' => $this->goods_id, 'goods_type' => CommonOrderDetail::TYPE_MALL_GOODS]);
        if (!$agent_goods) {
            $setting['is_alone'] = 0;
        ...
        foreach ($level_list as $item) {
            ...
            $newItem['agent']['agent_price'] = '';
            $newItem['over']['over_agent_price'] = '';
            if ($agent_goods) {
                $detail = AgentGoodsDetail::findOne(['agent_goods_id' => $agent_goods->id, 'level' => $item['level'], 'is_delete' => 0]);
                if ($detail) {
                    $newItem['equal']['equal_price'] = $detail->equal_price;
                    $newItem['agent']['agent_price'] = $detail->agent_price;
