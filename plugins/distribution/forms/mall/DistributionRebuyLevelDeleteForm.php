<?php
/**
 * Created by PhpStorm.
 * User: kaifa
 * Date: 2020-05-10
 * Time: 15:26
 */

namespace app\plugins\distribution\forms\mall;


use app\core\ApiCode;
use app\plugins\distribution\models\Distribution;
use app\plugins\distribution\models\DistributionLevel;
use app\models\BaseModel;
use app\plugins\distribution\models\RebuyLevel;

class DistributionRebuyLevelDeleteForm extends BaseModel
{


    public $id;

    public function rules()
    {
        return [
            [['id'], 'required'],
            [['id'], 'integer']
        ]; // TODO: Change the autogenerated stub
    }

    public function save()
    {

        if (!$this->validate()) {

            return $this->responseErrorInfo();
        }
        $level = RebuyLevel::findOne(['id' => $this->id, 'is_delete' => 0]);

        if (!$level) {
            return ['code' => ApiCode::CODE_FAIL, 'msg' => '所选择的复购等级不存在或已删除，请刷新后重试'];
        }

        $level->is_delete = 1;
        if (!$level->save()) {
            return $this->responseErrorMsg($level);

        }
        return ['code' => ApiCode::CODE_SUCCESS, 'msg' => '删除成功！'];

    }


}