...
class Card extends BaseActiveRecord{
    ...
    public static function generateCard($card_id){
        ...
        try{
            ...
            //$path = "/h5/#/?mall_id=".$card_info['mall_id']."&pid=".$card_info['user_id']."&source=".User::SOURCE_SHARE_POSTER;
            $path = "/h5/#/pages/user/shopping/shopping?mall_id=".$card_info['mall_id']."&pid=".$card_info['user_id']."&source=".User::SOURCE_SHARE_POSTER;
            $url = $host . $path;
            ...
            for($i = 1; $i <= $card_info['generate_num']; $i++){
                ...
                $use_code = self::_getUseCode($serialize_no);
    ...
    private static function _getSerializeNo($id, $num_length, $prefix=null){
        //默认前缀字母
        if(is_null($prefix)){
            $prefix = self::_getRandomString(2,'ABCDEFGHIJKLMNOPQRSTUVWXYZ');
        }
        //$id超出预设长度 等于 当前id长度
